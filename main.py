...
async def get_or_create_thread(user: types.User):
    """1000% надежный метод получения топика с проверкой через редактирование."""
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
    ...
    if thread_id:
        try:
            # Проверка живучести топика
            await bot.edit_forum_topic(ADMIN_GROUP_ID, thread_id, name=user.full_name[:120])
        except Exception:
            thread_id = None
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
    ...
    if not thread_id:
        try:
            # Формируем имя для названия топика
